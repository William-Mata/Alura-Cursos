...
using FormattingJson = Newtonsoft.Json.Formatting;
...
internal  class ByteBankAtendimento
{
    ...
    private void ExportarContas()
    {
        ...
        {
            Console.WriteLine("Digite 1 para formato JSON");
            Console.WriteLine("Digite 2 para formato XML");
            Console.Write("Informe sua opção: ");
            var opcao = Console.ReadLine();
            var caminhoArquivo = "../../../../Arquivos/";
            var nomeArquivo = opcao == "1" ? "contasExportadas.json" : "contasExportadas.xml";
           
            try
            {
                using (FileStream fileStream = new FileStream(caminhoArquivo + nomeArquivo, FileMode.Create)) // CRIAR O ARQUIVO
                {
                    using (StreamWriter streamwriter = new StreamWriter(fileStream)) 
                    {
                        if (opcao == "1")
                        {
                            string dadosArquivo = JsonConvert.SerializeObject(_listaDeContas, FormattingJson.Indented); // SERIALIZA A LISTA DE CONTA PARA JSON  
                            streamwriter.WriteLine(dadosArquivo); //  ESCREVE NO ARQUIVO OS DADOS SERIALIZADO
                        }
                        else if (opcao == "2")
                        {
                            var xmlSerializer = new XmlSerializer(typeof(List<ContaCorrente>)); // CRIA UMA INSTNACIA ESPECIFICANDO O TIPO DE INFORMAÇÃO QUE SERÁ SERALIZADA
                            xmlSerializer.Serialize(streamwriter, _listaDeContas); // SERIALIZA E ESCREVE NO ARQUIVO
                        }
                    }
                }
